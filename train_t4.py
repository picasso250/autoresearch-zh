...
Usage: uv run train_t4.py --dataset tinystorieszh
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        if USE_FLASH_ATTN:
            ...
            # PyTorch SDPA fallback for pre-Ampere GPUs such as T4.
            q = q.transpose(1, 2)
            k = k.transpose(1, 2)
            v = v.transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        # Keep low-precision embedding/value tables, but only use bf16 on supported GPUs.
        self.transformer.wte.to(dtype=LOW_PRECISION_DTYPE)
        for ve in self.value_embeds.values():
            ve.to(dtype=LOW_PRECISION_DTYPE)
...
if USE_COMPILE:
    ...
    print("torch.compile disabled for pre-Ampere compatibility.")
